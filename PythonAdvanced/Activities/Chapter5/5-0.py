...
class Calculator:
    ...
    def read_operations(self, statement):
        ...
        operator1 = self.digit_value(operand[next(operands_index)], statement)
        for current_operation in operations:
            try:
                operator2 = self.digit_value(operand[next(operands_index)], statement)
                ...
                result = self.do_operation(operator1, operator2, operation, statement)
                operator1 = result
    ...
    def digit_value(self, number_letter, statement):
        ...
        if number_letter.isdigit():
            ...
            try:
                ...
                print(
                    (
                        "'{0}' doesn't any assigned values ."
                        "You must added before using it."
                    ).format(number_letter)
                )
            else:
                return v
            ...
            print(
                (
                    "[ERROR] The sintaxis '{0}' is incorrect. "
    ...
    def add_letter(self, letter, value):
        try:
            ...
            print(
                (
                    "Letter '{0}' won't be agregated. " "It already exist in memory."
                ).format(letter)
            )
